...
var connectToDB = () => {
  try {
    connect(envVars_default.MONGODB_URI);
...
var InstructionSchema = new Schema2({
  stepTitle: {
    type: String,
    required: true,
    trim: true
...
var Item = model2("Item", ItemSchema);
...
var instructionSchema = z.object({
  stepTitle: z.string().trim().min(1, "Step title is required").max(100, "Step title is too long"),
  stepDescription: z.string().trim().min(1, "Step description is required").max(1e3, "Step description is too long")
});
...
var moreSchema = z.object({
  prep_time: z.string().trim().min(1, "Preparation time is required"),
  cook_time: z.string().trim().min(1, "Cooking time is required"),
  servings: z.string().trim().min(1, "Services info is required"),
...
var createItemSchema = z.object({
  ...
  thumbnail_image: z.string().url("Thumbnail must be a valid URL"),
  category: z.string().min(1, "Category is required"),
  description: z.string().min(30, "Description must be at least 30 characters").max(300, "Description must be at most 300 characters").optional(),
  instructions: z.array(instructionSchema).min(1, "At least one instruction is required"),
  tags: z.array(z.string()).optional(),
  ingredients: z.array(ingredientSchema, { required_error: "ingredient field is required" }).min(1, "At least one ingredient is required"),
...
var createCategorySchema = z2.object({
  name: z2.string().trim().toLowerCase().min(1, "Category name is required"),
  menuId: z2.number().int().positive("menuId must be a positive integer")
});
...
var newsletterSchema = new Schema3({
  name: {
    type: String,
    trim: true,
    minlength: 2,
...
var NewsletterSubscription = model3("NewsletterSubscription", newsletterSchema);
...
var newsletterSubscriptionSchema = z3.object({
  name: z3.string().trim().min(2, "Name is too short").max(50, "Name is too long").optional(),
  email: z3.string({ required_error: "Email is required" }).email("Invalid email format").trim().toLowerCase()
}).strict();
...
var ContactMessageSchema = new Schema4({
  firstName: {
    type: String,
    required: true,
    trim: true
...
var ContactMessage = model4("ContactMessage", ContactMessageSchema);
...
var contactMessageSchema = z4.object({
  firstName: z4.string({
